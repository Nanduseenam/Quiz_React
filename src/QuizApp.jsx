import React, { useState } from "react";
import { data } from "./data";
import './stylequiz.css';

export default function QuizApp() {

    const [index,setIndex] = useState(0);
    const [score, setScore] = useState(0);
    const [option, setOption] = useState('');
    const [finished, setFinish] = useState(false);

    const answers = ['Option1', 'Option1', 'Option3', 'Option2', 'Option4', 'Option2', 'Option1', 'Option2', 'Option3', 'Option3'];


    const handleOption = (optn) => {
        setOption(optn);
    }
    
    const handleNext = () => {
        if(index < data.length-1) {
            if(answers[index] === option) {
                setScore(score+1);
            }
            setIndex(index+1);
            setOption(null)
        } else {
           setFinish(true);
        }
    }
    if(finished) {
         return (
            <div className="scorePage">
            <h1>Quiz Finished....</h1>
            <h2>Your score is {score} out of {data.length}</h2>
            </div>
        )
    }
    return (
        <div className="quiz">
            <h1>KodQuiz</h1>
        <h3>{data[index].Question}</h3>
        <ul>
            <li className={option === 'Option1' ? 'selected' : ''}
            onClick={()=>{handleOption("Option1")}}
            >{data[index].Option1}</li>

            <li className={option === 'Option2' ? 'selected' : ''}
             onClick={()=>{handleOption("Option2")}}
            >{data[index].Option2}</li>

            <li className={option === 'Option3' ? 'selected' : ''}
             onClick={()=>{handleOption("Option3")}}
            >{data[index].Option3}</li>

            <li className={option === 'Option4' ? 'selected' : ''}
             onClick={()=>{handleOption("Option4")}}
            >{data[index].Option4}</li>

        </ul>
        <button onClick={handleNext} disabled= {!option}>Next</button>
        <h5>Question {index+1} of {data.length}</h5>
        </div>
    )
}